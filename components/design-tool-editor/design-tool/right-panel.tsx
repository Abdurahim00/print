...
export function RightPanel({ isMobile = false }: { isMobile?: boolean }) {
  ...
  const renderPanel = () => {
    ...
    if (!selectedProduct) {
      return (
        <div className="flex flex-col items-center justify-center h-full p-2 sm:p-4 md:p-6">
          <div className="text-center text-gray-500 max-w-full">
            <div className="w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 lg:w-20 lg:h-20 mx-auto mb-2 sm:mb-3 md:mb-4 lg:mb-6 rounded-full bg-gray-50 flex items-center justify-center flex-shrink-0">
              <Shirt className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 text-gray-400" />
            </div>
            <h3 className="text-xs sm:text-sm md:text-base lg:text-lg font-medium mb-1 sm:mb-1.5 md:mb-2 px-2">Start Your Design</h3>
            <p className="text-[10px] sm:text-xs md:text-sm text-gray-400 mb-2 sm:mb-3 md:mb-4 lg:mb-6 px-2">Select a product from the left toolbar to begin designing</p>
            <Button
              onClick={() => dispatch(setShowProductModal(true))}
              variant="outline"
              className="border-gray-200 text-gray-700 hover:bg-gray-50 text-[10px] sm:text-xs md:text-sm h-7 sm:h-8 md:h-9 lg:h-auto px-3 sm:px-4"
            >
              Browse Products
            </Button>
          </div>
  ...
  if (isMobile) {
    return (
      <div className="bg-white border-t border-gray-200 h-56 xs:h-60 sm:h-64 md:h-72 flex flex-col overflow-hidden">
        <div className="p-1.5 sm:p-2 md:p-3 border-b border-gray-200 flex-shrink-0">
          <Button
            onClick={handleSaveDesign}
            className="w-full bg-black hover:bg-gray-800 text-white transition-all duration-200 hover:scale-105 h-8 sm:h-9 md:h-10 text-xs sm:text-sm"
            size="sm"
            disabled={isSaving || !selectedProduct}
          >
            {isSaving ? (
              <>
                <Loader2 className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 mr-1 sm:mr-1.5 md:mr-2 animate-spin flex-shrink-0" />
                <span className="truncate">Saving...</span>
              </>
            ) : (
              <>
                <Save className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 mr-1 sm:mr-1.5 md:mr-2 flex-shrink-0" />
                <span className="truncate">Save Design</span>
              </>
            )}
          </Button>
        </div>
        <div className="flex-1 overflow-y-auto min-h-0 scrollbar-thin overscroll-contain">
          {renderPanel()}
        </div>
      </div>
    )
  ...
  return (
    <div className="w-64 md:w-72 lg:w-80 xl:w-96 bg-white/95 backdrop-blur-sm border-l border-gray-200/60 flex flex-col h-full shadow-xl overflow-hidden">
      <TopHeader />
      <div className="p-2 md:p-3 border-b border-gray-200 flex-shrink-0">
        <Button
          onClick={handleSaveDesign}
          className="w-full bg-black hover:bg-gray-800 text-white transition-all duration-200 hover:scale-105 text-sm"
          size="sm"
          disabled={isSaving || !selectedProduct}
        >
          {isSaving ? (
