...
export async function GET() {
  try {
    ...
    // Return orders with simplified items to reduce size
    const simplifiedOrders = orders.map(order => ({
      ...
      items: (order.items || []).map((item: any) => ({
        ...
        // Only include essential fields, not full design data
      })),
    ...
    console.log(`Fast endpoint returning ${simplifiedOrders.length} orders`)
