...
export default function CheckoutPage() {
  ...
  console.log('💰 Checkout Currency info:', { 
    original: currency, 
  ...
  const subtotal = cart.reduce((total, item) => {
    ...
    if (item.selectedSizes && item.selectedSizes.length > 0) {
      const sizeTotal = item.selectedSizes.reduce((sum, size) => sum + (size.price * size.quantity), 0);
      // Add design costs if present
      const designCost = item.designCosts?.totalDesignCost || 0;
      return total + sizeTotal + (designCost * item.quantity);
    }
    
    // Add design costs if present
    const designCost = item.designCosts?.totalDesignCost || 0;
    return total + (itemPrice * item.quantity) + (designCost * item.quantity);
  }, 0)
  ...
  const eligibleSubtotal = cart.reduce((total, item) => {
    ...
    if (item.selectedSizes && item.selectedSizes.length > 0) {
      ...
      pricePerUnit = parseFloat(String(item.price).replace(/[^\d.-]/g, '')) || 0
    }
    ...
    try {
      ...
      if (result.isValid) {
        ...
        toast({
          ...
          description: `You saved ${effectiveFormatPrice(result.discountAmount)} with coupon "${couponCode.toUpperCase()}"`,
          variant: "success",
        })
        setCouponCode("")
      } else {
  ...
  const handlePayment = async () => {
    ...
    if (!fullName || !email || !phone || !address || !city || !postalCode || !country) {
      ...
      items: cart.map((item) => {
        ...
        // Include ALL design-related data from cart item
          return {
            name: item.name,
          ...
          designPreview: (item as any).designPreview,
          ...
          // Design data - include all possible design properties
            designContext: (item as any).designContext,
          designContextDetails: (item as any).designContextDetails,
            designCanvasJSON: (item as any).designCanvasJSON,
          allDesignedAngles: (item as any).allDesignedAngles,
          finalItem: (item as any).finalItem,
          // Additional design properties that might exist
          ...
          // Include any other properties that might be in the cart item
          ...(item as any)
          };
      }),
    ...
    console.log('🛒 CHECKOUT: Cart items with design data:', cart.map(item => ({
      name: item.name,
      hasDesignContext: !!(item as any).designContext,
      hasCanvasJSON: !!(item as any).designCanvasJSON,
      hasAllDesignedAngles: !!(item as any).allDesignedAngles,
      hasDesignsArray: !!(item as any).designs,
      designsArrayLength: (item as any).designs?.length || 0,
      designContextKeys: (item as any).designContext ? Object.keys((item as any).designContext) : [],
      canvasObjectsCount: (item as any).designCanvasJSON?.objects?.length || 0,
      designs: (item as any).designs?.map((design: any) => ({
        angle: design.angle,
              ...
              <RadioGroup value={shippingOption} onValueChange={setShippingOption} className="space-y-3">
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">3-5 business days • {effectiveFormatPrice(79)}</span>
                  </div>
                </Label>
                <Label
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-gray-600 dark:text-gray-400 text-xs sm:text-sm">1-2 business days • {effectiveFormatPrice(149)}</span>
                  </div>
                </Label>
              </RadioGroup>
            </CardContent>
          ...
          <Card className="border border-black sm:border-2 dark:border-white bg-white dark:bg-gray-900 rounded-lg sm:rounded-xl overflow-hidden">
            ...
            <CardContent className="space-y-2 sm:space-y-3 p-3 sm:p-4 lg:p-6 text-sm sm:text-base">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-slate-900 dark:text-white">
                          {(() => {
                            ...
                            return effectiveFormatPrice(totalPrice);
                          })()}
                        </span>
                      </div>
                    </div>
              ...
              <div className="flex justify-between text-gray-700 dark:text-gray-300">
                ...
                <span className="font-semibold">{effectiveFormatPrice(subtotal)}</span>
              </div>
              {appliedCoupon && (
              ...
              <div className="flex justify-between text-gray-700 dark:text-gray-300">
                ...
                <span className="font-semibold">{effectiveFormatPrice(vatAmount)}</span>
              </div>
              <div className="flex justify-between text-gray-700 dark:text-gray-300">
                <span className="font-semibold">Shipping:</span>
                <span className="font-semibold">{effectiveFormatPrice(shippingCost)}</span>
              </div>
              <Separator className="bg-gray-300 dark:bg-gray-700" />
              <div className="flex justify-between font-black text-2xl text-black dark:text-white">
                <span>TOTAL:</span>
                <span>{effectiveFormatPrice(grandTotal)}</span>
              </div>
            </CardContent>
          </Card>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                        You're saving {effectiveFormatPrice(discountAmount)} on this order!
                      </p>
                    </div>
                    <Button
                      variant="outline"
          ...
          <Card className="border-2 border-black dark:border-white bg-white dark:bg-gray-900 rounded-xl overflow-hidden">
            ...
            <CardFooter className="p-6 pt-0">
              ...
              >
                ...
                {loading ? "PROCESSING..." : `COMPLETE ORDER • ${effectiveFormatPrice(grandTotal)}`}
              </Button>
            </CardFooter>
          </Card>
        </div>
