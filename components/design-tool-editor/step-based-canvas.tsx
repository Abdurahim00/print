...
export function StepBasedCanvas({ product, stepNumber, angle }: StepBasedCanvasProps) {
  ...
  // Use Redux design area directly and ensure re-renders
  const localDesignArea = designAreaCm2Redux
  const setLocalDesignArea = (area: number) => {
    dispatch(setDesignAreaCm2(area))
  }
  
  // Force re-render when Redux design area changes
  useEffect(() => {
    console.log('🔄 Design area changed in Redux:', designAreaCm2Redux)
    forceUpdate({})
  }, [designAreaCm2Redux])
  
  // Add direct canvas monitoring for real-time pricing updates
  useEffect(() => {
    let checkInterval: any = null
    
    // Function to safely get canvas from window
    const getCanvasSafe = () => {
      return typeof window !== 'undefined' ? (window as any).fabricCanvas : null
    }
    
    // Function to check if canvas is ready and set up event listeners
    const setupCanvasMonitoring = () => {
      ...
      events.forEach(event => {
        canvas.off(event, updatePricing)
      })
      
      // Add new event listeners
      events.forEach(event => {
        canvas.on(event, updatePricing)
      })
      
      // Store cleanup function
      ;(window as any)._canvasCleanup = () => {
        events.forEach(event => {
          canvas.off(event, updatePricing)
        })
      }
    ...
    const isSetup = setupCanvasMonitoring()
    
    // If not successful, set up an interval to keep trying
    if (!isSetup) {
      checkInterval = setInterval(() => {
        const success = setupCanvasMonitoring()
        if (success) {
    ...
    return () => {
      ...
      if ((window as any)._canvasCleanup) {
        (window as any)._canvasCleanup()
        delete (window as any)._canvasCleanup
      }
    }
  }, [])
  ...
  // Expose saveImmediately to window for external access
  useEffect(() => {
    if (typeof window !== 'undefined') {
      (window as any).saveCurrentDesign = saveImmediately
    }
  }, [saveImmediately])
  ...
  // Force save when component unmounts or step changes
  useEffect(() => {
    return () => {
      // Force save when leaving this step
      if (mountedRef.current) {
        saveStepDesign()
      }
    }
  }, [stepNumber, saveStepDesign])
  ...
  // Set up an interval to force updates for real-time pricing
  useEffect(() => {
    // Update every 100ms to ensure pricing stays current
  ...
  // Create a safe function to get design area that doesn't depend on getFabricCanvas
  const getDesignAreaSafe = () => {
    try {
      // Use window.fabricCanvas directly to avoid initialization issues
      const canvas = typeof window !== 'undefined' ? (window as any).fabricCanvas : null
      if (!canvas || canvas.disposed) return localDesignArea
      ...
      objects.forEach((obj: any) => {
        ...
        if (!boundingRect || boundingRect.width === 0 || boundingRect.height === 0) return
      ...
      console.warn('Error calculating design area:', error)
      return localDesignArea
    }
  }
  
  // Use the safe function in a memoized callback
  const getCurrentDesignArea = useCallback(() => {
    return getDesignAreaSafe()
  }, [localDesignArea])

  // Helper function to ensure consistent decimal precision
  const roundToTwoDecimals = useCallback((value: number) => {
    // Use a more precise rounding method to avoid floating point errors
    return Number((Math.round(value * 100) / 100).toFixed(2));
  }, []);

  // Calculate cumulative price including all steps with consistent precision
  const calculatePrice = useCallback(() => {
    try {
      const basePrice = roundToTwoDecimals(product?.price || 0);
      const pricePerCm2 = roundToTwoDecimals(product?.designCostPerCm2 || 0.5);
      
      // Get the running total from previous step
      const previousTotalKey = `design_${product.id}_step_${stepNumber - 1}_total`;
      let previousTotal = basePrice; // Start with base price for step 1
      
      if (stepNumber > 1) {
        const savedPreviousTotal = localStorage.getItem(previousTotalKey);
        if (savedPreviousTotal) {
          previousTotal = roundToTwoDecimals(parseFloat(savedPreviousTotal));
          console.log(`💰 Using previous step total: ${previousTotal.toFixed(2)} SEK`);
        } else {
          // If no previous total found, calculate cumulative from beginning
          let cumulativeTotal = basePrice;
          for (let step = 1; step < stepNumber; step++) {
            const areaKey = `design_${product.id}_step_${step}_area`;
            const savedArea = localStorage.getItem(areaKey);
            if (savedArea) {
              const area = roundToTwoDecimals(parseFloat(savedArea));
              const stepCost = roundToTwoDecimals(area * pricePerCm2);
              cumulativeTotal = roundToTwoDecimals(cumulativeTotal + stepCost);
            }
          }
          previousTotal = cumulativeTotal;
          console.log(`💰 Calculated cumulative total up to step ${stepNumber - 1}: ${previousTotal.toFixed(2)} SEK`);
        }
      }

      // Calculate current step's design cost
      let currentStepArea = 0;
      let currentStepCost = 0;
      
      if (stepNumber === 1) {
        // Step 1: base price + current step design cost
        currentStepArea = roundToTwoDecimals(getDesignAreaSafe());
        currentStepCost = roundToTwoDecimals(currentStepArea * pricePerCm2);
      } else {
        // Subsequent steps: previous total + current step design cost
        currentStepArea = roundToTwoDecimals(getDesignAreaSafe());
        currentStepCost = roundToTwoDecimals(currentStepArea * pricePerCm2);
      }

      // Calculate final running total
      const runningTotal = roundToTwoDecimals(previousTotal + currentStepCost);

      // Store this step's total for next step to use
      ...
      return roundToTwoDecimals(product?.price || 0);
    }
  }, [product?.price, product?.id, product?.designCostPerCm2, stepNumber, getDesignAreaSafe, roundToTwoDecimals]);
  ...
  useEffect(() => {
    ...
    const setupCanvasMonitoring = () => {
      ...
      const updateDesignArea = () => {
        ...
        // Force re-render to update price display immediately
        forceUpdate({})
        
        // Also trigger a price recalculation
        setTimeout(() => {
          if (mounted) {
            forceUpdate({})
          }
      ...
      // Add more specific scaling events for better responsiveness
      canvas.on('object:scaling', (e: any) => {
        console.log('📐 Object scaling detected - updating price immediately');
        // Immediate price update without debounce for scaling
        setTimeout(() => {
          if (mounted) {
            updateDesignArea();
          }
        }, 10); // Very short delay to ensure scaling is complete
      });
      ...
      canvas.on('mouse:move', (e: any) => {
        ...
        if (active && !isTransforming) {
          ...
          if (e.e.buttons === 1 && active) {
            ...
            // Start interval for continuous updates with higher frequency
            if (updateInterval) clearInterval(updateInterval)
            updateInterval = setInterval(() => {
              if (isTransforming && mounted) {
                updateDesignArea()
              }
            }, 25) // Update every 25ms for very smooth updates
          }
        }
        
        // If transforming and mouse is still down, continue updating
        if (isTransforming && e.e.buttons === 1 && mounted) {
          // Immediate update on mouse move for real-time feedback
          updateDesignArea()
        }
      })
                ...
                <button
                  onClick={() => {
                    if (confirm('Clear all designs for this product? This cannot be undone.')) {
                      // Clear all saved designs, areas, and totals for this product
                      for (let step = 1; step <= 4; step++) {
                        localStorage.removeItem(`design_${product?.id}_step_${step}`);
                        localStorage.removeItem(`design_${product?.id}_step_${step}_area`);
                        localStorage.removeItem(`design_${product?.id}_step_${step}_total`);
                      }
                      
                      // Clear the total price storage
                      localStorage.removeItem(`design_${product?.id}_total_price`);
                      ...
                      const basePrice = roundToTwoDecimals(product?.price || 0);
                      console.log('💰 Reset to base price:', basePrice);
