...
async function verifyProductSubcategories() {
  try {
    ...
    for (const product of productsData) {
      ...
      // Check category (field name is 'category' not 'categoryId')
      const categoryId = product.category?.toString() || product.categoryId?.toString();
      if (categoryId) {
      ...
      // Check both 'subcategory' and 'subcategoryId' fields
      if (product.subcategory && product.subcategory !== null) {
        subcategoryIds.push(product.subcategory.toString());
      }
      ...
      // Also check for array of subcategories
      if (product.subcategories && Array.isArray(product.subcategories)) {
        subcategoryIds = subcategoryIds.concat(product.subcategories.map(id => id ? id.toString() : null).filter(Boolean));
      }
      
      if (product.subcategoryIds && Array.isArray(product.subcategoryIds)) {
        subcategoryIds = subcategoryIds.concat(product.subcategoryIds.map(id => id ? id.toString() : null).filter(Boolean));
      }
